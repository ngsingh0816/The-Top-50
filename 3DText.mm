...
float distance(NSPoint p, NSPoint p2);
void normalize(Normal* normal);
...
/*+ (MDObject*) createText:(NSAttributedString *)str depth: (float)dep
{
	...
	calculating = TRUE;
	std::vector<NSPoint> points;
	NSAttributedString* text = [ str retain ];
...
/*
	...
	for (int z = 0; z < shapes.size() * 2; z++)
	{
		...
		[ face setDrawMode:shapes[z / 2].drawMode ];
		[ obj addFace:face ];
	...
	// Set the colors
	obj.objectColors[0].red = obj.objectColors[1].blue = obj.objectColors[2].red = obj.objectColors[2].green = 0.7;
	obj.objectColors[0].alpha = obj.objectColors[1].alpha = obj.objectColors[2].alpha = 1;
	[ obj setMidPoint:MakeThreePoint(0, 0, 0) ];
...
+ (NSNumber*) create2DText: (NSAttributedString*) text
{
	...
	return [ NSNumber numberWithUnsignedInt:img ];
}
